...
def parse_treeknit_summary(summary_path):
    ...
    row = {
        ...
        "n_subset_arg": grab(
            r"n_subset arg:\s*(\d+|None)", lambda x: None if x == "None" else int(x)
        ),
        ...
        "non_singleton_leaf_frac_K": grab(
            r"non-singleton leaf coverage / K:\s*([\d.]+)%", float
        ),
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        sys.exit(
            "Usage: python treeknit_summary_to_csv.py <results_summary.txt> <out.csv>"
        )
