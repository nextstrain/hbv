...
def is_missing(v):
    return (
        v in ("other", "NA", "") or v is None or (isinstance(v, float) and pd.isna(v))
    )
...
def monophyletic_label_clades(tree, label, node_to_label, args):
    ...
    for cl in tree.find_clades(order="preorder"):
        if cl._n_label > 0 and cl._n_other == 0:
            ...
            parent_pure = (
                parent is not None and parent._n_label > 0 and parent._n_other == 0
            )
            if not parent_pure:
                if (
                    args.min_count_mode == "use_for_annotation"
                    and args.min_count > cl._n_label
                ):
...
def assign_branch_clades(tree, node_data, args, fallback_annotations=None):
    ...
    for node in tree.find_clades(order="preorder"):
        if hasattr(node, "clade_label") and node.name not in (None, ""):
            ...
            if (
                args.min_count_mode == "use_for_branch_display"
                and len(node.get_terminals()) < args.min_count
            ):
                continue
            ...
            if args.only_monophyletic and args.branch_display_only_largest:
                if best_node_for_label.get(node.clade_label) is not node:
                    continue
            node_data["branches"][node.name] = {
                "labels": {args.nextclade_label: node.clade_label}
            }

        if fallback_annotations is not None:
            fallback_branch = fallback_annotations.get("branches", {}).get(
                node.name, {}
            )
            fallback_value = fallback_branch.get("labels", {}).get(
                args.fallback_annotation_nextclade_label
            )
            if fallback_value is not None:
                node_data["branches"][node.name] = {
                    "labels": {args.nextclade_label: fallback_value}
                }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--metadata", type=str, help="metadata file with subtype information"
    )
    parser.add_argument("--output", type=str, help="output file")
    parser.add_argument(
        "--min-count",
        type=int,
        help="subtypes to ignore if they have fewer than this many samples",
        default=5,
    )
    parser.add_argument(
        "--min-count-mode",
        type=str,
        help='mode to use for subtypes with fewer than min-count samples (options: "use_for_branch_display", "use_for_annotation")',
    )  # "use_for_pruning" config option has no effect here but is used in earlier script
    parser.add_argument(
        "--subtype-col",
        type=str,
        help="column in metadata file to use for subtype information",
    )
    parser.add_argument(
        "--nextclade-label", type=str, help="label to use for nextclade clades"
    )

    parser.add_argument(
        "--fallback-genotype-col",
        type=str,
        help="column in metadata file to use as clade if subtype information is missing",
        required=False,
    )
    parser.add_argument(
        "--fallback-annotation-file",
        type=str,
        help="file with fallback annotations",
        required=False,
    )
    parser.add_argument(
        "--fallback-annotation-nextclade-label",
        type=str,
        help="label to use for nextclade clades in fallback annotation file",
        required=False,
    )
    ...
    parser.add_argument(
        "--only_monophyletic",
        action="store_true",
        help="only store monophyletic clades; split labels into multiple monophyletic clades if needed",
    )
    parser.add_argument(
        "--branch_display_only_largest",
        action="store_true",
        help="whether to display branch labels multiple times on tree (if only-monophyletic is set) or only once for largest clade",
    )
    ...
    node_to_subtype = {
        x.Index: getattr(x, args.subtype_col) for x in metadata.itertuples()
    }
    ...
    tree, node_data = assign_clade_info(
        tree, node_data, args, fallback_annotations, node_to_subtype
    )
