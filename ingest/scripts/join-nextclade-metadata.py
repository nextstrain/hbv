...
def print_summary(name, n, total, fh=sys.stdout):
    ...
    print(
        f"{name:15}{stars:20} {round(n / total * 100, 1):5}% (n={n}/{total})", file=fh
    )
...
if __name__ == "__main__":
    ...
    metadata = pd.read_csv(
        args.metadata,
        index_col=METADATA_JOIN_COLUMN_NAME,
        sep="\t",
        low_memory=False,
        na_filter=False,
    )
    ...
    with open(args.summary, "w") as fh:
        ...
        aligned = result.drop(
            result[result["QC_overall_status"] == ""].index, inplace=False
        )

        genotypes = aligned.groupby(["clade_nextclade"])["QC_overall_status"].count()
        summarise(
            genotypes,
            "Genotypes assigned via Nextclade inference (all aligned sequences)",
            fh=fh,
        )

        has_genbank = (
            "genotype_genbank" in aligned.columns
            and aligned["genotype_genbank"].astype(str).str.strip().ne("").any()
        )

        if has_genbank:
            genotypes = aligned.groupby(["genotype_genbank"])[
                "genotype_genbank"
            ].count()
            summarise(
                genotypes,
                "Genotypes assigned via GenBank annotation (all aligned sequences)",
                fh=fh,
            )
            ...
            g = (
                aligned.groupby(["genotype_genbank", "clade_nextclade"])
                .size()
                .reset_index(name="n")
            )
            ...
            print(
                "No GenBank genotype annotations available in metadata; skipping GenBank vs Nextclade comparison.",
                file=fh,
            )
