...
def get_geno_from_match(match_result):
    if (
        "recombinant" in match_result
        or "Recombinant" in match_result
        or "/" in match_result
    ):
...
def parse_metadata(record):
    ...
    metadata["strain_name"] = (
        extract(source, "strain") or extract(source, "isolate") or "None"
    )
    metadata["country"] = extract(source, "country") or "None"
    metadata["host"] = extract(source, "host") or "None"
    metadata["genotype_genbank"], metadata["subgenotype_genbank"] = (
        parse_genotype(note) if note else ("None", "None")
    )
...
if __name__ == "__main__":
    ...
    if VERBOSE:
        print(
            f"{n_records} records parsed. Excluded "
            + ", ".join([f"n={v} ({k})" for k, v in exclude_counts.items()])
        )
        summarise(metadata)
